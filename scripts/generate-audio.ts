...
function resolveVoices(lang: string | undefined) {
    ...
    return {
        elevenlabs:
            (L && process.env[`ELEVENLABS_VOICE_${L}_ID`]) ??
            process.env.ELEVENLABS_VOICE_ID ??
            'EXAVITQu4vr4xnSDxMaL',
        openai:
            (L && process.env[`OPENAI_VOICE_${L}`]) ??
            process.env.OPENAI_VOICE ??
            'nova',
        gemini:
            (L && process.env[`GEMINI_VOICE_${L}`]) ??
            process.env.GEMINI_VOICE ??
            'Kore',
        piper:
            (L && process.env[`PIPER_VOICE_${L}`]) ??
...
async function generateOpenAI(
    ...
    const response = await client.audio.speech.create({
        model: (process.env.OPENAI_TTS_MODEL ?? 'tts-1-hd') as
            | 'tts-1'
            | 'tts-1-hd'
            | 'gpt-4o-mini-tts',
        voice,
...
async function generatePiper(
    ...
    const piperTool = path.resolve(
        __dirname,
        '../../../piper-tts/src/generate.ts',
    );

    if (process.env.PIPER_SERVER_URL) {
        const res = await fetch(
            `${process.env.PIPER_SERVER_URL.replace(/\/$/, '')}/tts`,
            {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
        ...
        if (!res.ok)
            throw new Error(
                `Piper server error ${res.status}: ${await res.text()}`,
            );
    ...
    if (piper.status !== 0) {
        console.error('Piper stdout:', piper.stdout);
        console.error('Piper stderr:', piper.stderr);
        console.error('Model path:', modelPath);
        throw new Error(
            `Piper synthesis failed: ${piper.stderr || piper.stdout}`,
        );
    }
    ...
    if (ff.status !== 0) {
        console.error('FFmpeg error:', ff.error);
        console.error('FFmpeg stdout:', ff.stdout);
        console.error('FFmpeg stderr:', ff.stderr);
        throw new Error(
            `WAV→MP3 failed: ${ff.error?.message || ff.stderr || ff.stdout}`,
        );
    }
}
...
function getAudioDurationMs(filePath: string): number {
    try {
        ...
        return Math.ceil(
            parseFloat(
                (JSON.parse(out) as {format: {duration: string}}).format
                    .duration,
            ) * 1000,
        );
    } catch {
...
async function main(): Promise<void> {
    ...
    if (!flowPath) {
        console.error('Usage: tsx src/generate-audio.ts <flow.json>');
        process.exit(1);
    }
    ...
    if (PROVIDER === 'openai' && !process.env.OPENAI_API_KEY) {
        console.error(
            'Missing OPENAI_API_KEY in .env (or set TTS_PROVIDER=elevenlabs)',
        );
        process.exit(1);
    }
    ...
    if (PROVIDER === 'piper') {
        ...
        const {spawnSync} = await import('child_process');
        const check = spawnSync('python', ['-c', 'import piper'], {
            encoding: 'utf-8',
        });
        if (check.status !== 0) {
            console.error(
                'piper-tts not installed. Run: cd tools/piper-tts && ./setup.sh',
            );
            process.exit(1);
        }
    }
    ...
    console.log(
        `🎙  Provider: ${label} — ${narrationSteps.length} narrations to generate\n`,
    );
    ...
    for (const step of flow.steps) {
        ...
        process.stdout.write(
            `🎙  [${narrationIndex}/${narrationSteps.length}] (${pct}%) [${step.id}] Generating... `,
        );
        ...
        if (PROVIDER === 'openai') {
            ...
            await generatePiper(
                normalizeForPiper(step.narration),
                audioPath,
                voices.piper,
            );
        } else {
